...
static uint8_t ioReadLoop(ReadFn_t readFn, uint16_t len)
{
    /*uint8_t data;
...
static uint8_t ioWriteLoop(WriteFn_t writeFn, uint16_t len)
{
/*    uint8_t data;
...
static uint8_t ioRead2Loop(Read2Fn_t readFn, uint16_t len)
{
    /*uint8_t data[2];
...
static uint8_t ioWrite2Loop(Write2Fn_t writeFn, uint16_t len)
{
    /*uint8_t data[2];
...
static uint8_t ioReadNibLoop(uint16_t len, bool earlyExit)
{
/*    uint16_t i;
    uint8_t data;
...
static uint8_t ioWriteNibLoop(uint16_t len)
{
    /*uint16_t i;
    uint8_t data, *ptr;
 ...
 * next function to run. nib_parburst_write_checked() sets this flag.
 */
static uint8_t nib_parburst_read_checked(void)
{
    /*if (!suppressNibCmd)
